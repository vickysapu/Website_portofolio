...
<body class="nav-fixed bg-light">
            ...
            <main>
                <div class="container-xl p-5">
                    <div class="d-flex justify-content-between align-items-end">
                        <h2 class="display-6 mb-0">Data Skill</h2>
                    </div>
                    <hr class="mt-2 mb-5" />
                    <button class="btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#modalTambahSkill">Tambah Skill</button>

                    <div class="card card-raised h-100 overflow-hidden">
                        <div class="card-header bg-primary text-white px-4">
                            <div class="fw-500">Skill Terdaftar</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <thead class="text-center font-monospace">
                                    <tr>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Nama Skill</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-center">Gambar Logo</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Progres</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Aksi</td>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <?php
                                    $query = $kon->query("SELECT * FROM skill");

                                    while ($row = $query->fetch_assoc()) {
                                    ?>
                                        <tr>
                                            <td class="px-4 border-top"><?php echo !empty($row['nama_skill']) ? $row['nama_skill'] : '-'; ?></td>
                                            <td class="px-4 border-top"><img src="../assets/gambar_skill/<?php echo !empty($row['gambar_skill']) ? $row['gambar_skill'] : 'error.jpg' ?>" width="50px" height="50px" alt=""></td>
                                            <td class="px-4 border-top">
                                                <?php echo !empty($row['progres']) ? $row['progres'] . "%" : '-'; ?>
                                            </td>
                                            <td class="px-4 border-top">
                                                <a href="../assets/hapusskill.php?id_skill=<?php echo $row['id_skill'] ?>" class="btn" onclick="return confirm('Aakah anda yakin untuk menghapus skill ini?')">
                                                    &#10006;
                                                </a>
                                            </td>
                                        </tr>
                ...
                <div class="container-xl p-5">
                    <div class="d-flex justify-content-between align-items-end">
                        <h2 class="display-6 mb-0">Data Testimoni</h2>
                    </div>
                    <hr class="mt-2 mb-5" />

                    <div class="card card-raised h-100 overflow-hidden">
                        <div class="card-header bg-primary text-white px-4">
                            <div class="fw-500">Testimoni Terdaftar</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                <tbody class="align-middle"></tbody>
                                <thead class="text-center font-monospace">
                                    <tr>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-start">Foto</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-center">Nama</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Pesan</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Status</td>
                                    </tr>
                                </thead>

                                <tbody class="text-center">
                                    <?php
                                    $query = $kon->query("SELECT * FROM testimoni");

                                    while ($row = $query->fetch_assoc()) {
                                    ?>
                                        <tr>
                                            <td class="px-4 border-top">
                                                <img src="../assets/foto_test/<?php echo !empty($row['foto']) ? $row['foto'] : 'profile_error.jpg' ?>" width="50px" height="50px" alt="">
                                            </td>
                                            <td class="px-4 border-top">
                                                <?php echo !empty($row['nama']) ? $row['nama'] : '-'; ?>
                                            </td>
                                            <td class="px-4 border-top">
                                                <?php echo !empty($row['pesan']) ? $row['pesan'] : '-'; ?>
                                            </td>
                                            <td class="px-4 border-top">
                                                <?php
                                                if ($row['ket_status'] === '0') {
                                                    echo '<a href="../assets/updatePostTestimoni.php?id=' . $row['id_testimoni'] . '"><span style="color: red;">&#x2716;</span> pending</a>';
                                                } else {
                                                    echo '<a href="../assets/updatePostTestimoni.php?id=' . $row['id_testimoni'] . '"><span style="color: green;">&#x2713;</span> post</a>';
                                                }
                                                ?>
                                            </td>
                                        </tr>
                ...
                <div class="container-xl p-5">
                    <div class="d-flex justify-content-between align-items-end">
                        <h2 class="display-6 mb-0">Pesan Masuk</h2>
                    </div>
                    <hr class="mt-2 mb-5" />
                    <div class="card card-raised h-100 overflow-hidden">
                        <div class="card-header bg-primary text-white px-4">
                            <div class="fw-500">Pesan Terdaftar</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                <tbody class="align-middle"></tbody>
                                <thead class="text-center font-monospace">
                                    <tr>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-start">Nama Pengirimm</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-center">Email Pengirim</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Pesan Pengirim</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Waktu Terkirim</td>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <?php
                                    $query = $kon->query("SELECT * FROM pesan");
                                        ...
                                        <tr>
                                            <td class="px-4 border-top text-start">
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <?php echo !empty($row['nama_pengirim']) ? $row['nama_pengirim'] : '-'; ?>
                                                </div>
                                            </td>
                                            <td class="px-4 border-top"><?php echo !empty($row['email_pengirim']) ? $row['email_pengirim'] : '-'; ?></td>
                                            <td class="px-4 border-top"><?php echo !empty($row['pesan']) ? $row['pesan'] : '-'; ?></td>
                                            <td class="px-4 border-top"><?php echo !empty($row['waktu_terkirim']) ? $row['waktu_terkirim'] : '-'; ?></td>
                                        </tr>
                                    <?php
                                    }
                                    ?>
                ...
                <div class="container-xl p-5">
                    ...
                    <div class="card card-raised h-100 overflow-hidden">
                        <div class="card-header bg-primary text-white px-4">
                            <div class="fw-500">Medsos Terdaftar</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                <tbody class="align-middle"></tbody>
                                <thead class="text-center font-monospace">
                                    <tr>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-start">Link Akun</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted text-center">Akun Medsos</td>
                                        <td class="px-4 py-2 border-bottom-0 text-muted">Aksi</td>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <?php
                                    $query = $kon->query("SELECT * FROM medsos");
    ...
    <div class="modal fade" id="modalTambahSkill" tabindex="-1" aria-labelledby="modalTambahSkillLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTambahSkillLabel">Tambah Skill</h5>
                    <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="../assets/tambah_skill.php" method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="gambar-skill">Logo Skill</label>
                            <input type="file" class="form-control" name="gambar_skill" id="gambar-skill" aria-describedby="emailHelp" required>
                        </div>
                        <div class="form-group">
                            <label for="nama_skill">Nama Skill</label>
                            <input type="text" class="form-control" name="nama_skill" id="nama_skill" placeholder="nama skill..." required>
                        </div>
                        <div class="form-group">
                            <label for="progres">progres</label>
                            <input type="number" class="form-control" id="progres" name="progres" aria-describedby="emailHelp" placeholder="progres...">
                        </div>
